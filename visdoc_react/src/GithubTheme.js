import React from 'react';
import './github_theme.css';
import { Routes, Route } from 'react-router-dom';
import MyForm from './MyForm';
import ResponsePage from './ResponsePage';
import VidCreatorPage from './VidCreatorPage';

export default function GithubTheme() {
  return (
    <div className='main-container'>
      <div className='w-light'>
        <div className='banner'>
          <div className='link' />
          <div className='flex-row-faa'>
            <div className='vector' />
            <div className='qbsearch-input'>
              <input className='input' />
            </div>
            <span className='product'>Product</span>
            <span className='solutions'>Solutions</span>
            <span className='resources'>Resources</span>
            <span className='open-source'>Open Source</span>
            <span className='enterprise'>Enterprise</span>
            <span className='pricing'>Pricing</span>
            <span className='sign-in'>Sign in</span>
            <span className='sign-up'>Sign up</span>
            <div className='vector-3' />
            <div className='vector-4' />
            <div className='vector-5' />
            <div className='vector-6' />
            <div className='vector-7' />
            <div className='rectangle' />
            <div className='rectangle-8' />
          </div>
        </div>
        <div className='background'>
          <div className='flex-row-fa'>
            <span className='slash'>/</span>
            <button className='link-signed-in'>
              <span className='notifications'>Notifications</span>
              <div className='vector-9' />
            </button>
            <button className='link-a'>
              <div className='overlay'>
                <span className='k'>27.3k</span>
              </div>
              <span className='fork'>Fork </span>
              <div className='vector-b' />
            </button>
            <button className='star-repository'>
              <div className='users-starred'>
                <span className='span-k'>165k</span>
              </div>
              <div className='div-vector' />
              <span className='span-star'> Star </span>
            </button>
            <span className='span-flutter'>flutter</span>
            <span className='span-flutter-c'>flutter</span>
            <div className='div-vector-d' />
            <span className='span-public'>Public</span>
          </div>
          <div className='div-nav-repository'>
            <div className='div-list'>
              <div className='div-item-link'>
                <span className='span-code'>Code</span>
                <div className='div-flex-column-f'>
                  <div className='div-svg'>
                    <div className='div-vector-e' />
                  </div>
                </div>
              </div>
              <div className='div-item-link-f'>
                <span className='span-issues'>Issues</span>
                <button className='button-overlay'>
                  <span className='span-k-plus'>5k+</span>
                </button>
                <div className='div-svg-10' />
              </div>
              <div className='div-item-link-11'>
                <span className='text-16'>Pull requests</span>
                <button className='Button-5'>
                  <span className='text-17'>159</span>
                </button>
                <div className='group-7'>
                  <div className='pic-7' />
                </div>
              </div>
              <div className='group-8'>
                <span className='text-18'>Actions</span>
                <div className='group-9'>
                  <div className='img-9' />
                </div>
              </div>
              <div className='section-5'>
                <span className='text-19'>Projects</span>
                <button className='Button-6'>
                  <span className='text-1a'>23</span>
                </button>
                <div className='section-6'>
                  <div className='img-a' />
                </div>
              </div>
              <div className='wrapper-9'>
                <span className='text-1b'>Wiki</span>
                <div className='wrapper-a'>
                  <div className='img-b' />
                </div>
              </div>
              <div className='box-5'>
                <span className='text-1c'>Security</span>
                <div className='svg-12'>
                  <div className='vector-13' />
                </div>
              </div>
              <div className='item-link'>
                <span className='insights'>Insights</span>
                <div className='svg-14'>
                  <div className='vector-15' />
                </div>
              </div>

              <div className='div-item-link_doc'>
                <span className='span-doc'>Documentation</span>
                <div className='div-flex-column-f'>
                  <div className='div-svg'>
                    <div className='div-vector-doc' />
                  </div>
                  <div className='div-horizontal-divider' />
                </div>
              </div>

            </div>
          </div>
        </div>
        <Routes>
          <Route path="/" element={<MyForm />} />
          <Route path="/response-page" element={<ResponsePage />} />
          <Route path="/creator-page" element={<VidCreatorPage />} />

        </Routes>
        <div className='flex-row-f'>
          <div className='vector-16' />
          <span className='copyright'>© 2024 GitHub, Inc.</span>
          <span className='terms'>Terms</span>
          <span className='privacy'>Privacy</span>
          <span className='security'>Security</span>
          <span className='status'>Status</span>
          <span className='docs'>Docs</span>
          <span className='contact'>Contact</span>
          <span className='manage-cookies'>Manage cookies</span>
          <span className='do-not-share'>
            Do not share my personal information
          </span>
        </div>
      </div>
    </div>
  );
}
